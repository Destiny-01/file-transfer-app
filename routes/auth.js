...
router.post("/register", async (req, res) => {
  ...
  const checkUser = await User.findById(blah.user._id);
  if (checkUser) {
    req.status = "error";
...
router.get("/verifyEmail/:email/:verification_token", async (req, res) => {
...
router.get("/resetPassword/:email/:reset_token", async (req, res) => {
  const email = req.params.email;
